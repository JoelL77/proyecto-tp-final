...
export class Mapa {
      ...
      this.calcularRuta(solicitudRuta)
        .then((respuesta) => {
          const enlaceMapa = `https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(
            ...
            .map((index:any) => encodeURIComponent(puntosIntermedios[index].location.lat + ',' + puntosIntermedios[index].location.lng))
            .join('%7C')}&dirflg=d`;
          resolve(enlaceMapa);
        })
        .catch((error) => {
  ...
  calcularRuta(solicitudRuta: any): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      directionsService.route(solicitudRuta, (respuesta:any, estado:any) => {
        if (estado === 'OK') {
          resolve(respuesta);
        } else {
